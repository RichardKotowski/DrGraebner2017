...
BEGIN
{
  ...
  $postmaster        = 'webmaster@DrGraebner.de';
  @referers          = qw(drgraebner.de);
  @allow_mail_to     = qw(rezept@DrGraebner.de kontakt@DrGraebner.de Praxis@DrGraebner.de);
  @recipients        = ();
  %recipient_alias   = ();
...
sub send_main_email_body_header {
  ...
  $self->mailer->print(<<END);
EMail von der Internetseite www.DrGraebner.de
erstellt durch $self->{FormConfig}{realname} $self->{FormConfig}{email}
erstellt am $date
$dashes
END
}
